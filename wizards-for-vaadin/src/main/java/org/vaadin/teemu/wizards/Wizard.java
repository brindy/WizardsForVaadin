 ...
 * 
 * <p>
 * The steps of the wizard must be implementations of the {@link WizardStep} interface. Use the {@link #addStep(WizardStep)} method to add these steps in the
 * same order they are supposed to be displayed.
 * </p>
 * 
 * <p>
 ...
 * {@link #setUriFragmentEnabled(boolean)} method. Each step will get a generated identifier that is used as the URI fragment. If you wish to override these
 * with your own identifiers, you can add the steps using the overloaded {@link #addStep(WizardStep, String)} method.
 * </p>
 * 
 * <p>
 * To react on the progress, cancellation or completion of this {@code Wizard} you should add one or more listeners that implement the
 * {@link WizardProgressListener} interface. These listeners are added using the {@link #addListener(WizardProgressListener)} method and removed with the
...
public class Wizard extends CustomComponent implements UriFragmentChangedListener {
	...
	/**
	 * Sets a {@link Component} that is displayed on top of the actual content. Set to {@code null} to remove the header altogether.
	 * 
	 * @param newHeader
	 *            {@link Component} to be displayed on top of the actual content or {@code null} to remove the header.
	 */
	...
	/**
	 * Adds a step to this Wizard with the given identifier. The used {@code id} must be unique or an {@link IllegalArgumentException} is thrown. If you don't
	 * wish to explicitly provide an identifier, you can use the {@link #addStep(WizardStep)} method.
	 * 
	 * @param step
	 * @param id
	...
	/**
	 * Adds a step to this Wizard. The WizardStep will be assigned an identifier automatically. If you wish to provide an explicit identifier for your
	 * WizardStep, you can use the {@link #addStep(WizardStep, String)} method instead.
	 * 
	 * @param step
	 */
	...
	public void updateButtons() {
		...
		} else if (currentStep instanceof AdvancedWizardStep) {
	...
	/**
	 * Triggers a {@link WizardCompletedEvent} if the current step is the last step and it allows advancing (see {@link WizardStep#onAdvance()}). This method is
	...
	/**
	 * Activates the next {@link WizardStep} if the current step allows advancing (see {@link WizardStep#onAdvance()}) or calls the {@link #finish()} method the
	 * current step is the last step. This method is called when user clicks the next button.
	 */
	public void next() {
		if (isLastStep(currentStep)) {
			finish();
	...
	/**
	 * Activates the previous {@link WizardStep} if the current step allows going back (see {@link WizardStep#onBack()}) and the current step is not the first
	...
	/**
	 * Removes the given step from this Wizard. An {@link IllegalStateException} is thrown if the given step is already completed or is the currently active
	 * step.
	 * 
	 * @param stepToRemove
	 *            the step to remove.
	...
	/**
	 * Removes the step with given id from this Wizard. An {@link IllegalStateException} is thrown if the given step is already completed or is the currently
	 * active step.
	 * 
	 * @param id
